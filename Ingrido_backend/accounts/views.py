...
def GetRecipesByCity(request):
    ...
    if city_name:
        recipes = recipes.filter(city__name__iexact=city_name)
    if search_query:
...
def get_dashboard_recipes(request):
    ...
    serializer = RecipeListSerializer(sampled_recipes, many=True, context={'request': request})
    return Response(serializer.data)
...
def saved_recipes(request):
    ...
    serializer = SavedRecipeSerializer(bookmarks, many=True, context={'request': request})
    return Response(serializer.data)
...
def regenerate_meal_plan(request):
    return generate_and_save_meal_plan(request._request)
