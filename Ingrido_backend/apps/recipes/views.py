...
# FIX 1: Module-level groq_client hataya — ab sirf get_groq_client() use hoga
# Pehle yahan Groq() seedha ban raha tha jo import time par .env read karta tha
# Agar key baad mein set hoti to yeh None rehta tha permanently
...
def get_dashboard_recipes(request):
    ...
    FIX 2: order_by('?') hataya — woh full table scan karta tha har request par.
    Ab Python-side shuffle with session seed use ho raha hai.
...
def get_ai_recipe_detail(request, recipe_title):
    ...
    # FIX 1: get_groq_client() use karo — module-level client nahi
    groq_client = get_groq_client()
...
def get_ai_substitute(request, pk=None):
    """
    Pure AI substitute - AI reads recipe and decides what to suggest.
    FIX 4: Ab get_groq_client() use ho raha hai — module-level None check nahi
    """
    recipe_title = request.data.get('recipe_title')
    ingredient = request.data.get('ingredient', '').strip()
    ...
    # FIX 4: Har request par fresh client lo — module-level wala stale hota tha
    groq_client = get_groq_client()
...
def search_ai_recipes_list(request):
    ...
    # FIX 1: get_groq_client() use karo
    groq_client = get_groq_client()
    ...
    try:
        ...
        for idx, item in enumerate(ai_recipes):
            # FIX 3: Stable ID banao — query + index se, random nahi
            # Pehle random.randint tha jis se har search par naya ID milta tha
            stable_id = hashlib.md5(f"{query}-{item.get('title', idx)}".encode()).hexdigest()[:8]
            item['id'] = f"ai-{idx}-{stable_id}"
            item['is_ai_generated'] = True
            item['is_saved'] = False
